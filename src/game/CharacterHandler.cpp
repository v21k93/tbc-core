...
>>>>>>> 3392afbf1533fbd6720058e84083b0fb3e5a39bc
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADINVENTORY,       "SELECT data,bag,slot,item,item_template,FakeEntry,FakeOwner FROM character_inventory JOIN item_instance ON character_inventory.item = item_instance.guid WHERE character_inventory.guid = '%u' ORDER BY bag,slot", GUID_LOPART(m_guid));
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder * holder)
{
    ...
    if (!pCurrChar->getCinematic())
    {
        ...
        if (ChrRacesEntry const* rEntry = sChrRacesStore.LookupEntry(pCurrChar->getRace()))
        {
			...
			if (sWorld.getConfig(CONFIG_PLAYER_START_FIRST) == 1)
