...
struct event_npcAI : public ScriptedAI
{
    //*** HANDLED FUNCTION ***
    //This is the constructor, called only once when the creature is first created
    event_npcAI(Creature *c) : ScriptedAI(c) {}
...
CreatureAI* GetAI_event_npc(Creature* pCreature)
{
    return new event_npcAI (pCreature);
}

//This function is called when the player clicks an option on the gossip menu
void SendDefaultMenu_event_npc(Player* pPlayer, Creature* pCreature, uint32 uiAction)
{
...
//This function is called when the player clicks an option on the gossip menu
bool GossipSelect_event_npc(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    if (uiSender == GOSSIP_SENDER_MAIN)
        SendDefaultMenu_event_npc(pPlayer, pCreature, uiAction);

    return true;
}

//This function is called when the player opens the gossip menu
bool GossipHello_event_npc(Player* pPlayer, Creature* pCreature)
{
...
void AddSC_event_npc()
{
    ...
    newscript->Name = "event_npc";
    newscript->GetAI = &GetAI_event_npc;
    newscript->pGossipHello = &GossipHello_event_npc;
    newscript->pGossipSelect = &GossipSelect_event_npc;
