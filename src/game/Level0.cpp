...
bool ChatHandler::HandleXpRateCommand(const char* args)
{
	...
	int32 xpRate = atoi((char*)args);

	if (xpRate < 1 || xpRate >15)
	{
		PSendSysMessage(LANG_XP_ERROR);
        SetSentErrorMessage(true);
        return false;
	}

	CharacterDatabase.PExecute("UPDATE characters SET xp_rate = '%u' WHERE guid = %u", xpRate, chr->GetGUID());
	chr->ChangeXPRate(xpRate);

	PSendSysMessage(LANG_XP_SUCCESS, xpRate);
	SetSentErrorMessage(true);
